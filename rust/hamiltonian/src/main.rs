...
impl Node {
    ...
    fn retain(&mut self, selected: &[&str]) {
        self.links.retain(|x| selected.contains(&&*x.as_str()));
    }
}
...
impl Graph {
    fn link<I: Into<String>>(&mut self, root: String, links: impl IntoIterator<Item = I>) {
        let root_node = self.nodes.entry(root).or_insert(Node::mk());
        for l in links {
            root_node.add(l.into());
        }
    }
    fn link_str(&mut self, root: &str, links: Vec<&str>) {
        self.link(root.to_string(), links)
    }
    fn reduce(&mut self, selected: &[&str]) {
        self.nodes.retain(|k, _| selected.contains(&&*k.as_str()));
        for node in self.nodes.values_mut() {
            node.retain(selected);
        }
    }
    fn find_hamiltonian(&self, starting_node: &str) -> Option<Vec<String>> {
        self.find_hamiltonian_r(
            starting_node,
            &mut Vec::with_capacity(self.nodes.len()),
            &mut HashSet::with_capacity(self.nodes.len() * 2),
        )
    }
    fn find_hamiltonian_r<'a>(
        ...
        if path.len() == self.nodes.len() {
            return Some(path.iter().map(|x| x.to_string()).collect());
        } else {
            for node in self
                .nodes
                .get(starting_node)
                ...
                .flat_map(|x| x.links.iter())
            {
                if !visited.contains(&&*node.as_str()) {
                    let r = self.find_hamiltonian_r(node, path, visited);
                    if r.is_some() {
                        return r;
                    }
                }
